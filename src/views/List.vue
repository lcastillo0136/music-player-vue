        ...
        <section class="vbox animated fadeInUp">
          <section class="scrollable hover">
            <div class="list-group no-radius no-border no-bg m-t-n-xxs m-b-none auto"> 
              <a href="#" class="list-group-item" v-bind:class="{'active': selected.genre == g}"  v-for="g in genresList" v-on:click.stop.prevent="selected.genre=g;selected.page=1" :key="g"> {{g}} </a> 
            </div>
          </section>
        </section>
      </aside> <!-- / side content -->
                ...
                <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2" :key="'picture-' + index">
                  <div class="item">
                    ...
                    <div class="padder-v"> 
                      ...
                      <router-link :to="'/songs/'+s.id" class="text-ellipsis text-xs text-muted">{{s.artist | artistTags}}</router-link>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  data: () => ({
    selected: {
      genre: localStorage.getItem('list.config.selected.genre') || 'All',
      page: 1,
      action:'',
    },
  ...
  watch: {
    selected: {
      handler: function() {
        localStorage.setItem('list.config.selected.genre',this.selected.genre);
  ...
  computed: {
    ...
    songList() {
      return this.$store.getters.filterByGenre(this.selected.genre, this.filter);
    },
    songCount() {
      return this.songList.length.toString();
    },
    pagelimit() {
      return new Array(Math.ceil(this.songList.length / 18)).slice(Math.max(this.selected.page - 5,0), Math.max(Math.min(this.selected.page + 6, Math.ceil(this.songList.length / 18)),10)).fill(true).map((e,ei)=>Math.max(this.selected.page-5,0)+ei+1);
    },
  },
  methods: {
    play(song) {
