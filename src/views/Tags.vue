        ...
        <section class="vbox animated fadeInUp">
          <section class="scrollable hover">
            <div class="list-group no-radius no-border no-bg m-t-n-xxs m-b-none auto"> 
              <router-link v-for="g in tagList" :to="'/songs/tags/'+g" class="list-group-item" v-bind:class="{'active': $route.params.tag == g}" :key="g">{{g}}</router-link>
            </div>
          </section>
        </section>
      </aside> <!-- / side content -->
...
export default {
  ...
  data: () => ({
   selected: {
      tag: localStorage.getItem('list.config.selected.tag') || 'All',
      page: 1,
      action:'',
    },
  ...
  computed: {
    ...
    songList() {
      return this.$store.getters.filterByGenre("", "").filter(f=>f.tags.includes(this.$route.params.tag)||this.$route.params.tag=='All');
    },
    songCount() {
